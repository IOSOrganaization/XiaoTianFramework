...
public class MyCircularSlider: UIControl{
    ...
    public override func draw(_ rect: CGRect) {
        ...
        if let labelsEvenSpacing = labelsInnerMarking{
            ...
            for (index, label) in labelsEvenSpacing.enumerated().reversed(){
                ...
                text.draw(in: labelLocation, withAttributes: [NSFontAttributeName:labelFont, NSForegroundColorAttributeName:labelColor])
            }
        }
        //
        context?.saveGState()
    ...
    func labelSize(_ label:NSString)-> CGSize{
        let rect = label.boundingRect(with: CGSize(width:CGFloat.greatestFiniteMagnitude, height:CGFloat.greatestFiniteMagnitude), options:.usesLineFragmentOrigin, attributes: [NSFontAttributeName: labelFont], context:nil)
        return rect.size
    }
